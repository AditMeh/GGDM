...
class DiffusionModel(nn.Module):
    def __init__(self, sizes, img_shape):
        ...
        self.encoder_layers, self.decoder_layers = create_network(sizes, *img_shape)
...
block = lambda sin,sout : nn.Sequential(*[nn.Conv2d(sin, sout, 3, 2, 1), nn.BatchNorm2d(sout), nn.ReLU()])

upsample_block = lambda sin,sout : nn.Sequential(*[nn.ConvTranspose2d(sin, sout, 3, 2, 1, 1), nn.BatchNorm2d(sout), nn.ReLU()])
...
if __name__ == "__main__":
    ...
    net = UNet(image_channels = 1, n_blocks = 1, is_attn = (False, False, True, True)).to(device)
