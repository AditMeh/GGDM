...
class ContextUnet(nn.Module):
    def __init__(self, in_channels, n_feat=256, n_classes=10):
        ...
        self.up1 = UnetUp(6 * n_feat, n_feat)
    ...
    def forward(self, x, c, t, context_mask):
        ...
        up2 = self.up1(torch.cat((up1, cemb1.expand(-1, -1, up1.shape[-2], up1.shape[-1])), 1) + temb1, down2)
        up3 = self.up2(torch.cat((up2, cemb2.expand(-1, -1, up2.shape[-2], up2.shape[-1])), 1) + temb2, down1)
            ...
            for w_i, w in enumerate(ws_test):
                ...
                if ep % 5 == 0 or ep == int(n_epoch-1):
                    ...
                    def animate_diff(i, x_gen_store):
                        print(
                            f'gif animating frame {i} of {x_gen_store.shape[0]}', end='\r')
                        plots = []
                    ...
                    ani.save(
                        save_dir + f"gif_ep{ep}_w{w}.gif", dpi=100, writer=PillowWriter(fps=5))
